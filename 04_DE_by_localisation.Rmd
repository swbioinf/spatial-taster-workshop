...
**Whats the difference between plasma cells in the lymphoid regions vs those in the lamina propria?** 



Ie. We expect there to be some differce in cell state of plamsa cells 

We want to know if the plasma


* Niche 6 captures the lymphoid regions seen in in HC_a, full of T cells and plasma cells
* Niche 3 coveres the non-epitheial regions in the lamina propria, and has a sizesable number of plasma cells also.
...
Each niche has a differenct balance of celltypes within it - and we can again see the sizeable proprortion of plasma cells present in both n3 and n6.
...
We only care about plasma cells, and mostly only in n3 and n6 ;  so we can set a thresold accordingly. Here we choose 50 cells.
...
Only testing for differnece in plasma cells located in the B/T cell rich lymphoid features (n6) vs those in the epithelial/stromal mix of the laminar propria layer (n3).
...
Note that is only 4 samples for niche 6 - not every sample had lymphoid regions to begin with.
...
How many (plasma) pools per niche and group?
...
```{r}
plotSpatialFeature( sfe.sample.HC, "MS4A1",colGeometryName = "cellSeg") + 
  theme(legend.title=element_blank()) +
  ggtitle(sample)
